...
function render(items) {
  list.innerHTML = items.length ? items.map((post) => `<article class="post"><div class="post-meta"><span>${post.category || '随笔'}</span><time>${post.date || ''}</time><span>${post.minutes || ''}</span></div><h3>${post.title || '未命名文章'}</h3><p>${post.excerpt || ''}</p><div class="markdown">${markdown(post.body)}</div><div class="post-tags">${post.tags.map((tag) => `<button data-tag="${tag}"># ${tag}</button>`).join('')}</div></article>`).join('') : '<p class="empty">没有找到相关文章。</p>';
